...
#key_file2=open("Server_public_key.bin", "rb")

#server_public_key=key_file2.read()
#c=key_file.read()
...
##SSL##
cli_SSL=crypto.load_pkcs12(open("Cliente.p12","rb").read(),"1234")#abrir ficheiro do cliente
...
#private_key_cli_SSL=cert_cryptography.private_key()
#public_key_cli_SSL=cert_cryptography.public_key()#chave publica do cliente a partir do cert

CA_cer = crypto.load_certificate(crypto.FILETYPE_ASN1,open('CA.cer','rb').read())#certificado CA
...
class Client:
    ...
    def process(self, msg=b"-"):
        ...
        if msg.startswith(test2):
            ...
            cert_serv =cert_serv_txt #crypto.dump_certificate(crypto.FILETYPE_PEM, cert_serv_txt)#converte para pem o certificado do serv
            ...
            #cert_serv_object = crypto.dump_certificate(crypto.FILETYPE_ASN1,cert_cryptography_s)
            #cert_serv_pem_load_pem = crypto.dump_certificate(crypto.FILETYPE_PEM, CA_cer)
            #store.add_cert(cert_serv_pem)

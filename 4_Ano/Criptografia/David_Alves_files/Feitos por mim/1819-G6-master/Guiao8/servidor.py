...
serv_SSL=crypto.load_pkcs12(open("Servidor.p12","rb").read(),"1234")#abrir ficheiro do servidor
...
#private_key_serv_SSL=serv_SSL.get_privatekey()#chave privada do servidor 
private_key_serv_SSL= serialization.load_pem_private_key(private_key_pem, None, default_backend())

#public_key_serv_SSL=cert_cryptography.public_key()#chave publica do servidor a partir do cert
#public_key_serv_SSL_pem=crypto.dump_publickey(crypto.FILETYPE_PEM, public_key_serv_SSL)#chave publica em pem
...
store.add_cert(CA_cer)#certificado CA adicionado a store
...
class ServerWorker(object):
    ...
    def process(self, msg):
        ...
        if msg.startswith(stbyte1):
            ...
            #cert_cli = crypto.load_certificate(crypto.FILETYPE_PEM,cert_cli_txt)#crypto.dump_certificate(crypto.FILETYPE_PEM, cert_cli_txt)
            ...
            result_client=store_c.verify_certificate()

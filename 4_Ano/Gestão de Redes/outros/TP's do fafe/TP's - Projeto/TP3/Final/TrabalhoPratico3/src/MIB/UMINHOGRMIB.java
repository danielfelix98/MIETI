...
public class UMINHOGRMIB implements MOGroup {
    ...
    // Tables
    public static final OID oidRandomEntry = new OID(new int[]{1, 3, 6, 1, 3, 1, 1, 5, 1});

    // Index OID definitions
    public static final OID oidRandomKey = new OID(new int[]{1, 3, 6, 1, 3, 1, 1, 5, 1, 1});
    public static final OID oidRandomNumber = new OID(new int[]{1, 3, 6, 1, 3, 1, 1, 5, 1, 2});
    ...
    protected void createMO(MOFactory moFactory) {
        ...
        unpredictableCommunity =
                moFactory.createScalar(oidUnpredictableCommunity,
                        moFactory.createAccess(MOAccessImpl.ACCESSIBLE_FOR_READ_ONLY),
                        null,
                        TC_MODULE_SNMPV2_TC, TC_DISPLAYSTRING);
        unpredictablePort =
                moFactory.createScalar(oidUnpredictablePort,
                        moFactory.createAccess(MOAccessImpl.ACCESSIBLE_FOR_READ_ONLY),
                        new Integer32());
        unpredictableSeed =
                moFactory.createScalar(oidUnpredictableSeed,
                        moFactory.createAccess(MOAccessImpl.ACCESSIBLE_FOR_READ_ONLY),
                        null,
                        TC_MODULE_SNMPV2_TC, TC_DISPLAYSTRING);
    ...
    public MOScalar<OctetString> getUnpredictableCommunity() {
        return unpredictableCommunity;
    }

    public MOScalar<Integer32> getUnpredictablePort() {
        return unpredictablePort;
    }

    public MOScalar<OctetString> getUnpredictableSeed() {
    ...
    public void registerMOs(MOServer server, OctetString context) throws DuplicateRegistrationException {
        ...
        server.register(this.unpredictableCommunity, context);
        server.register(this.unpredictablePort, context);
        server.register(this.unpredictableSeed, context);
    ...
    public void unregisterMOs(MOServer server, OctetString context) {
        ...
        server.unregister(this.unpredictableCommunity, context);
        server.unregister(this.unpredictablePort, context);
        server.unregister(this.unpredictableSeed, context);
    ...
    public static class RandomEntryRow extends DefaultMOMutableRow2PC {
        ...
        public OctetString getRandomKey() {
            //--AgentGen BEGIN=randomEntry::getRandomKey
            //--AgentGen END
            return (OctetString) super.getValue(idxRandomKey);
        }

        public void setRandomKey(OctetString newColValue) {
            //--AgentGen BEGIN=randomEntry::setRandomKey
            //--AgentGen END
            super.setValue(idxRandomKey, newColValue);
        }

        public OctetString getRandomNumber() {
            //--AgentGen BEGIN=randomEntry::getRandomNumber
            //--AgentGen END
            return (OctetString) super.getValue(idxRandomNumber);
        }

        public void setRandomNumber(OctetString newColValue) {
        ...
        public void setValue(int column, Variable value) {
            ...
            switch (column) {
                case idxRandomKey:
                    setRandomKey((OctetString) value);
                    break;
                case idxRandomNumber:
                    setRandomNumber((OctetString) value);
                    break;
                default:
    ...
    class RandomEntryRowFactory implements MOTableRowFactory<RandomEntryRow> {
        public synchronized RandomEntryRow createRow(OID index, Variable[] values) throws UnsupportedOperationException {
            RandomEntryRow row = new RandomEntryRow(index, values);
